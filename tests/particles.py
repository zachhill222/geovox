...
print(f"P.center = Vector(0,0,0) -> P.bbox= {repr(P.bbox)}")
...
point = Vector(4,0,1e-5)
print(f"P.contains({repr(point)})= {P.contains(point)}")

# PRISM
print(f"\n\n=============== TESTING Prism =============== ")
P = Prism(Vector(1,2,3), Vector(0,0,0), Quaternion(1, Vector(0,0,0)))
print(f"P= {repr(P)}")
print(P)
print(f"P.volume= {P.volume}")
print(f"P.bbox= {repr(P.bbox)}")
P.center = Vector(1,2,3)

print("\nTESTING CENTER")
print(f"P.center = Vector(1,2,3) -> P.bbox= {repr(P.bbox)}")
print(P)
...
print("\nTESTING RADII")
newR = Vector(1,sqrt(2),3)
P.R = newR
print(f"P.R= {repr(newR)} -> P.bbox= {repr(P.bbox)}")
print(P)

print("\nTESTING QUATERNION")
newQ = Quaternion(1,Vector(0,0,0)).setrotation(PI/2, Vector(0,0,1))
P.Q = newQ
...
point = P.vertex(0)
print(f"P.contains({repr(point)})= {P.contains(point)}")
point += Vector(1e-5,-1e-5,0)
print(f"P.contains({repr(point)})= {P.contains(point)}")

point = P.facecenter(4)
print(f"P.contains({repr(point)})= {P.contains(point)}")
point += Vector(0,0,-1e-5)
print(f"P.contains({repr(point)})= {P.contains(point)}")
...
# ELLIPSOID
print(f"\n\n=============== TESTING Ellipsoid =============== ")
P = Ellipsoid(Vector(1,2,3), Vector(0,0,0), Quaternion(1, Vector(0,0,0)))
print(f"P= {repr(P)}")
print(P)
print(f"P.volume= {P.volume}")
print(f"P.bbox= {repr(P.bbox)}")
...
P = SuperEllipsoid(Vector(1,2,3), 1, 1, Vector(0,0,0), Quaternion(1, Vector(0,0,0)))
print(f"P= {repr(P)}")
print(P)
print(f"P.volume= {P.volume}")
print(f"P.bbox= {repr(P.bbox)}")
...
SElist = []
for i in range(n):
	SElist.append(SuperEllipsoid(Vector(1,2,3), 1, 1, Vector(0,0,0), Quaternion(1, Vector(0,0,0))))
toc = time()
